...
function findJsonFiles(dir) {
  ...
  for (const item of items) {
    const fullPath = path5.join(dir, item.name);
    if (item.isDirectory()) {
...
function findSessionFile(sessionsDir, shortId) {
  ...
  return files.find((f) => path6.basename(f) === `${shortId}.json`) || null;
}
...
async function sessionFinalize(sessionId, cwd, transcriptPath, cleanupPolicy, graceDays) {
  const mnemeDir = path6.join(cwd, ".mneme");
  const sessionsDir = path6.join(mnemeDir, "sessions");
  const sessionLinksDir = path6.join(mnemeDir, "session-links");
  if (!fs7.existsSync(mnemeDir)) {
    return { status: "skipped" };
  }
  let sessionFile = findSessionFile(sessionsDir, sessionId) || (sessionId.length > 8 ? findSessionFile(sessionsDir, sessionId.slice(0, 8)) : null);
  if (!sessionFile) {
    const linkFile = fs7.existsSync(
      path6.join(sessionLinksDir, `${sessionId}.json`)
    ) ? path6.join(sessionLinksDir, `${sessionId}.json`) : path6.join(sessionLinksDir, `${sessionId.slice(0, 8)}.json`);
    if (fs7.existsSync(linkFile)) {
  ...
  if (transcriptPath && fs7.existsSync(transcriptPath)) {
    try {
  ...
  const now = nowISO();
  if (sessionFile && fs7.existsSync(sessionFile)) {
    ...
    if (!hasSummary) {
      ...
      if (cleanupPolicy === "immediate") {
        ...
        if (cleanupResult.deleted) {
          ...
          fs7.unlinkSync(sessionFile);
          const linkFile = fs7.existsSync(
            path6.join(sessionLinksDir, `${sessionId}.json`)
          ) ? path6.join(sessionLinksDir, `${sessionId}.json`) : path6.join(sessionLinksDir, `${sessionId.slice(0, 8)}.json`);
          if (fs7.existsSync(linkFile)) {
            fs7.unlinkSync(linkFile);
          }
          console.error(
  ...
  const sessionLinkFile = fs7.existsSync(
    path6.join(sessionLinksDir, `${sessionId}.json`)
  ) ? path6.join(sessionLinksDir, `${sessionId}.json`) : path6.join(sessionLinksDir, `${sessionId.slice(0, 8)}.json`);
  if (fs7.existsSync(sessionLinkFile)) {
    ...
    if (link.masterSessionId) {
      const masterFile = findSessionFile(sessionsDir, link.masterSessionId);
      if (masterFile && fs7.existsSync(masterFile)) {
