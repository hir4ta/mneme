        ...
        <div className="grid gap-4 md:grid-cols-2">
          {phases.map((phase, index) => (
            <Card key={phase.name} className="relative overflow-hidden">
              <div className="absolute top-0 left-0 w-1 h-full bg-stone-800 dark:bg-stone-600" />
              <CardContent className="p-4 pl-5">
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-6 h-6 rounded-full bg-stone-800 dark:bg-stone-700 text-white text-xs flex items-center justify-center">
                    {index + 1}
                  </span>
        ...
        <div className="relative">
          ...
          <div className="space-y-4">
            {dataFlowSteps.map((step, index) => (
              <div key={step.event} className="flex gap-4 relative">
                <div className="w-8 h-8 rounded-full bg-white dark:bg-stone-900 border-2 border-stone-300 dark:border-stone-600 flex items-center justify-center text-xs font-medium z-10">
                  {index + 1}
                </div>
